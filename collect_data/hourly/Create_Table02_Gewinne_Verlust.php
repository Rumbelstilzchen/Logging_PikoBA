...
$sql="drop table if exists `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRgewinne`;\n";
$sql.="create table `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRgewinne` as select
`T1`.`time_sec` AS `time_sec`,`T1`.`TIMESTAMP` AS `TIMESTAMP`,
round(sum(`T1`.`Einspeisen_kWh` * `T1`.`verkauf_cent_kWh`) / 100,5) AS `Verkauf`,
round(sum((`T1`.`HomeConsumptionSolar_kWh` + `T1`.`HomeConsumptionBat_kWh`) * `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten`.`cent_kWh`) / 100,5) AS `Ersparnis`,
...
round(sum(`T1`.`HomeConsumptionGrid_kWh` * `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten`.`cent_kWh`) / 100,5) AS `BezugKosten`,
`$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRsteuerl`.`SteuerlAbschr` AS `Abschreibung`
from (
	select
	...
	from ((
		...
		left join `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten` on(DATE(`$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRdaily`.`TIMESTAMP`) >= `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten`.`DATE` and DATE(`$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRdaily`.`TIMESTAMP`) <= `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten`.`Valid_UNTIL`))
	...
	left join `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten` on (DATE(`T1`.`TIMESTAMP`) >= `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten`.`DATE` and DATE(`T1`.`TIMESTAMP`) <= `$mysql_PV_data_DB_name`.`$mysql_PV_data_tablename_WRstromkosten`.`Valid_UNTIL`)
